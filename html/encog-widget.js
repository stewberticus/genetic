...
ENCOG.GUI.TSP.prototype =
{
    ...
    reset:function () {
        'use strict';
        this.cities =[];
        var feats = MAP.pointLayer.getSource().getFeatures();
        for(var i in feats){
                   this.cities[i] = mercator2latlon(feats[i].getGeometry().getCoordinates());
        }
...
//        }
    ...
    pathDistance:function (path, i1, i2) {
        return MAP.distArray[path[i1]][path[i2]];
        //return this.distance(this.cities[path[i1]], this.cities[path[i2]]);
    },
    distance:function(lonlat1,lonlat2){
            var lat1 = lonlat1[1];
            var lon1 = lonlat1[0];
            var lat2 = lonlat2[1];
            var lon2 = lonlat2[0];
